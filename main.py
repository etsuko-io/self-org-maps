...
class Main:
    ...
    def __init__(self):
        self.input_path = "input-images/shibuya.png"
        self.img_width = math.ceil(self.BASE_WIDTH / 18)
        self.img_height = math.ceil(self.BASE_HEIGHT / 18)
        self.epochs = 5
        ...
        self.radius_sqs = [
            # round(avg_dim / 1),
            round(self.avg_dim / 2),
            round(self.avg_dim / 4),
            round(self.avg_dim / 8),
            # round(math.pow(avg_dim / 7, 2)),
            # round(math.pow(avg_dim / 3, 2)),
        ]
        self.lr_decay = 0.1
        self.radius_decay = 0.1
        self.proj = self.init_project()
    ...
    def init_project(self) -> Project:
        ...
        return Project(name=self.get_project_name(), parent_dir=parent_dir)
